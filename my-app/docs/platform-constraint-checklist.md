...
> **You are the Platform Engineer for ButwalHacks.com.**
> This document enumerates every hard ceiling, timeout, payload limit, and connection boundary
> that the project must respect. Read this before deploying any new API route, server action,
> or external integration. If a change touches any ceiling described here, the PR must
> document the constraint check.
...
8. [Database Connection Audit](#8-database-connection-audit)
9. [Critical Gaps Summary](#9-critical-gaps-summary)
10. [Deployment Gate Checklist](#10-deployment-gate-checklist)
...
|---------|-------|------------------|------------|
| **Execution timeout** | 300s (5 min) | 800s (13 min), 1800s beta | All 27 API routes + 21 server actions |
...
| Query pagination | ❌ Not implemented | Routes return unbounded result sets: `notifications`, `events`, `badges`, `projects`, `registrations`. |
...
- `ai-team-match.ts` → Groq (no timeout, no retry)  
...
|-----|----------------|--------|-----|
| **No production logging** | All | Errors invisible in production | Install logging service (Axiom, Better Stack) |
...
| **No body size limits** | All 17 mutation routes | ✅ **FIXED** — All 16 POST routes (15 body-parsing + webhook) reject > 1 MB via `rejectOversized(request)` helper in `validation.ts` before any body parsing. Returns 413. |
...
| **Server actions throw Errors** | All 21 actions | Generic 500 with no error detail | Return structured `{ success, error }` objects |
...
- [ ] All mutation routes have rate limiting applied via `withRateLimit` wrapper (or a `ponytail:` comment documenting why not)
- [ ] All mutation routes that create resources have idempotency key support (or a comment documenting why not)
...
- [ ] Every external `fetch()` has: timeout, error handling, retry strategy (or comment why not)
