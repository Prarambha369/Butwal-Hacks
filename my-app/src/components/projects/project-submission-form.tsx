...
export default function ProjectSubmissionForm() {
  ...
  // ponytail: Fetch hacker's registered events for the event selector
  useEffect(() => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      <div className="lg-surface p-8 rounded-3xl border border-glass shadow-2xl animate-[fadeInUp_0.4s_ease-out]">
        <div className="flex items-center gap-3 mb-8">
          <div className="p-3 rounded-2xl bg-bh-red-600 text-primary">
            <Code2 className="w-6 h-6" />
          </div>
          <div>
            <h2 className="text-2xl font-bold text-primary">Submit Your Project</h2>
            <p className="text-sm text-secondary">Show the community what you&apos;ve built.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <button 
            ...
            type="submit" 
            className="w-full py-4 bg-bh-red-600 hover:bg-bh-red-500 text-primary rounded-2xl font-bold transition-all flex items-center justify-center gap-2 shadow-lg shadow-bh-red-500/20"
          >
            {isSubmitting ? (
              <RoseSpinner size="sm" />
            ) : (
