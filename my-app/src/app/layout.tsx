...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (        <html lang="en" suppressHydrationWarning className={`${dmSans.variable} ${jetbrainsMono.variable}`}>
        <head>
          ...
          {/* Favicons, Apple touch icon, theme-color + manifest link are
              generated by Next.js from `metadata`/`viewport` above — no manual
              <link rel="icon"> tags (avoids stale 404s to deleted assets). */}
          </head>
