...
import { Briefcase, EyeOff, Eye, Trash2, Edit3, Clock, MapPin, DollarSign } from "lucide-react";
...
const TYPE_COLORS: Record<string, string> = {
  ...
  other: "bg-surface/20 text-secondary border border-border",
};
    ...
    return (
      <div className="bh-card p-12 text-center">
        ...
        <a
          ...
          className="mt-6 inline-flex items-center gap-2 rounded-full bg-bh-red-500 px-6 py-2.5 text-sm font-bold text-white hover:bg-deep-red transition-all"
        >
          Create Opportunity
        </a>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex gap-1.5 bg-surface/10 rounded-xl p-1 border border-border">
          {["all", "job", "internship", "grant", "bounty", "other"].map((type) => (
            <button
              ...
              className={cn(
                ...
                filterType === type ? "bg-primary-red/20 text-primary-red" : "text-secondary hover:text-primary"
              )}
            >
              {type}
            </button>
        ...
        <select
          ...
          className="bg-surface/10 border border-border rounded-xl px-3 py-1.5 text-xs text-secondary focus:outline-none"
        >
      ...
      {filtered.map((opp) => (
        <div
          key={opp.id}
          className={cn(
            "bh-card border p-5 transition-all",
            opp.is_active ? "border-border" : "border-border/30 opacity-60"
          )}
        >
          <div className="flex items-start justify-between gap-4">
              ...
              {opp.skills_required.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {opp.skills_required.slice(0, 5).map((skill) => (
                    <span key={skill} className="px-2 py-0.5 rounded-md bg-surface/10 border border-border text-[10px] text-secondary">
                      {skill}
                    </span>
                  ))}
                </div>
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              <button
                ...
                className="p-2 rounded-lg bg-surface/10 border border-border text-secondary hover:text-primary transition-all disabled:opacity-40"
                title={opp.is_active ? "Deactivate" : "Activate"}
              >
                {opp.is_active ? <EyeOff size={14} /> : <Eye size={14} />}
              </button>
              <a
                href={`/portal/bounties/${opp.id}/edit`}
                className="p-2 rounded-lg bg-surface/10 border border-border text-secondary hover:text-status-blue transition-all"
                title="Edit"
              >
                <Edit3 size={14} />
              </a>
              <button
                ...
                disabled={loadingId === opp.id}
                className="p-2 rounded-lg bg-surface/10 border border-border text-secondary hover:text-primary-red transition-all disabled:opacity-40"
                title="Delete"
              >
                <Trash2 size={14} />
              </button>
