...
-- operations. Authentication is handled by Auth0. RLS is unnecessary
-- because:
--   1. Every API route and server action authenticates via Auth0 before
...
-- Drop the legacy auth-specific RLS function since we no longer use it
